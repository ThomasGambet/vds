...
class Agenda extends Table
{
    ...
    {
        $sql = <<<EOD
			Select id, nom, date_format(date, '%d/%m/%Y') as dateFr, if(date < current_date, 1, 0) as old, type, modifierPar, ajouterPar
            From agenda
            Order by date desc;
EOD;
...
EOD;
    ...
    // Récupération des enregistrements à afficher (la date doit être supérieure à la date du jour)
    public static function getLesEvenementsAVenirMembre()
    {
        $sql = <<<EOD
		    Select nom, date_format(date, '%d/%m/%Y') as dateFr, description
            From agenda
